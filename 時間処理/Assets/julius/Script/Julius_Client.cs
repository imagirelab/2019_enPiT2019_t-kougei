...
public class Julius_Client : MonoBehaviour {
    ...
    {
        text = GetComponentInChildren<mic_debug>();
		...
		}catch(System.ComponentModel.WarningException w){
			Debug.Log("Not Found." + w);
            text.setText("Not Found." + w);
			return false;
		}
		return true;
	}
	...
	private void kill_julius_server(){
		...
		if (julius_process.HasExited) {
            ...
            text.setText("Kill julius server.");
		} else {
			julius_process.Kill();
		}
	...
	private IEnumerator start_julius_server(){
        ...
        text.setText("Julius Initialize...");
		yield return new WaitForSeconds(wait_time);
		Debug.Log ("Connect start");
        text.setText("Connect start");
		...
		if (run) {
			if (connect) {
				...
				*/
                ...
                text.setText("tmp : " + tmp + " " + "word : " + words + " Result : " + Result);
				if(tmp != words){
					words  = tmp;
					Result = tmp;
				}else{
                ...
                text.setText("Wait for response...");
			}
		} else {
			Debug.Log ("Error");
            text.setText("Error");
